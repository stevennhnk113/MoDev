...
namespace PersonalApp
{
	public class ItemsDataAccess
	{
		...
		public List<Item> PostAndGet(int randomNum, string type, int level)
		{
			var formContent = new FormUrlEncodedContent(new[]
			{
				new KeyValuePair<string, string>("random", randomNum.ToString()),
				new KeyValuePair<string, string>("charactertype", type),
				new KeyValuePair<string, string>("characterlevel", level.ToString()),
			});
			...
			Task<HttpResponseMessage> task = request.PostAsync("http://thursdayhomeworkpost.azurewebsites.net/api/GetItems", formContent);
			task.Wait();

			List<Item> tempList = new List<Item>();
			JObject itemJobject = JObject.Parse(task.Result.Content.ReadAsStringAsync().Result);
			Item tempItem = null;
			if (itemJobject.First.First.Value<int>() == 0)
			{
				foreach (var each in itemJobject.Last.First)
			...
			DeleteAllItems();
			foreach(Item apiItem in tempList)
			{
				SaveItem(apiItem);
			}
			return tempList;
			...
			else
			{
				lock (collisionLock)
				{
					var query = from item in database.Table<Item>()
								select item;
					return query.AsEnumerable().ToList<Item>();
				}
			}
		}
